...
class CartoontoReal(tfds.core.GeneratorBasedBuilder):
  """CycleGAN dataset."""
  MANUAL_DOWNLOAD_INSTRUCTIONS ="Manually download CartoontoReal"
  ...
  def _info(self):
    return tfds.core.DatasetInfo(
        ...
        features=tfds.features.FeaturesDict({
            ...
            "image_cartoon":tfds.features.Image(),
            # "label": tfds.features.ClassLabel(names=["R", "C"]),
        }),
        supervised_keys=("image", "image_cartoon"),
  ...
  def _split_generators(self, dl_manager):
    ...
    train_a_path = os.path.join("/Main/Filtered/trainR")
    train_b_path = os.path.join("/Main/Filtered/trainC")
    test_a_path = os.path.join("/Main/Filtered/testR")
    test_b_path = os.path.join("/Main/Filtered/testC")

    return [
        tfds.core.SplitGenerator(
            name="trainR", gen_kwargs={
                "path": train_a_path,
                "label": "R",
            }),
        # tfds.core.SplitGenerator(
        #     name="trainC", gen_kwargs={
        #         "path": train_b_path,
        #         "label": "C",
        #     }),
        tfds.core.SplitGenerator(
            name="testR", gen_kwargs={
                "path": test_a_path,
                "label": "R",
            }),
        # tfds.core.SplitGenerator(
        #     name="testC", gen_kwargs={
        #         "path": test_b_path,
        #         "label": "C",
        #     }),
    ]
  ...
  def _generate_examples(self, path, label):
    ...
    if "train" in path:
      cartoon_path = "/Main/Filtered/trainC"
    else:
      cartoon_path = "/Main/Filtered/testC"
    for image in images:
      record = {
          "image": os.path.join(path, image),
          "image_cartoon":os.path.join(cartoon_path, image),
          # "label": label,
      }
